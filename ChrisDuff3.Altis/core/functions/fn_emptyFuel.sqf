...
while{true} do 
{
    ...
    if(isEngineOn _vehicleToFuel && ((driver _vehicleToFuel) == player) && !(_vehicleToFuel isKindOf "Air") && (_vehicleToFuel != player) ) then
    {
        ...
        {
            hint "Votre réservoir d'éssence est vide. Allez à la station-service la plus proche!";
        }
        else
        {
            if(fuel _vehicleToFuel < 0.03) then
            {
                hint "J'espère que vous pouvez courir - parce que vous venez de manqué d'éssence!";
            };
        };
    };
    sleep 2;
